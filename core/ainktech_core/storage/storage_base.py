...
from ainktech_core.brain.info import StorageInfo
from ainktech_core.storage.local_storage import ainktechFile
...
class StorageBase(ABC):
    ...
    async def get_files(self) -> list[ainktechFile]:
        """
        Abstract asynchronous method to get the files `ainktechFile` in the storage.

        Returns:
            list[ainktechFile]: A list of ainktechFile objects representing the files in the storage.
    ...
    async def upload_file(self, file: ainktechFile, exists_ok: bool = False) -> None:
        ...
        Args:
            file (ainktechFile): The file to upload.
