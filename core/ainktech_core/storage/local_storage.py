...
from ainktech_core.brain.serialization import LocalStorageConfig, TransparentStorageConfig
from ainktech_core.files.file import ainktechFile
...
class LocalStorage(StorageBase):
    ...
    Attributes:
        ...
        files (list[ainktech File]): A list of files stored in this local storage.
    ...
    Args:
        dir_path (Path | None): Optional directory path for storing files.
                                Defaults to the environment variable `ainktech_LOCAL_STORAGE`
                                or `~/.cache/ainktech/files`.
    ...
    def __init__(self, dir_path: Path | None = None, copy_flag: bool = True):
        self.files: list[ainktechFile] = []
        self.hashes: Set[str] = set()
        self.copy_flag = copy_flag

        if dir_path is None:
            self.dir_path = Path(
                os.getenv("ainktech_LOCAL_STORAGE", "~/.cache/ainktech/files")
            )
        else:
    ...
    async def upload_file(self, file: ainktechFile, exists_ok: bool = False) -> None:
        """
        ...
        Args:
            file (ainktechFile): The file object to upload.
    ...
    async def get_files(self) -> list[ainktechFile]:
        """
        Retrieves the list of files stored in the local storage.

        Returns:
            list[ainktechFile]: A list of stored file objects.
        """
        return self.files
    ...
    def load(cls, config: LocalStorageConfig) -> Self:
        ...
        tstorage.files = [ainktechFile.deserialize(f) for f in config.files.values()]
        return tstorage
...
class TransparentStorage(StorageBase):
    ...
    def load(cls, config: TransparentStorageConfig) -> Self:
        tstorage = cls()
        tstorage.id_files = {
            i: ainktechFile.deserialize(f) for i, f in config.files.items()
        }
        return tstorage
