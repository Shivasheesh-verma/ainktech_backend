...
from ainktech_core.rag.prompts import custom_prompts
from ainktech_core.rag.utils import (
    combine_documents,
    format_file_list,
    get_chunk_metadata,
...
class ainktechQARAG:
    """
    ainktechQA RAG is a class that provides a RAG interface to the ainktechQA system.
    """
    ...
    def filter_history(
        ...
        Takes in a chat_history= [HumanMessage(content='Qui est Chloé ? '), AIMessage(content="Chloé est une salariée travaillant pour l'entreprise ainktech en tant qu'AI Engineer, sous la direction de son supérieur hiérarchique, Stanislas Girard."), HumanMessage(content='Dis moi en plus sur elle'), AIMessage(content=''), HumanMessage(content='Dis moi en plus sur elle'), AIMessage(content="Désolé, je n'ai pas d'autres informations sur Chloé à partir des fichiers fournis.")]
    ...
    def build_chain(self, files: str):
        """
        Builds the chain for the ainktechQA RAG.
        """
    ...
    def answer(
        ...
        question: str,
        history: ChatHistory,
        list_files: list[ainktechKnowledge],
        metadata: dict[str, str] = {},
    ) -> ParsedRAGResponse:
        """
        Answers a question using the ainktechQA RAG synchronously.
        """
    ...
    async def answer_astream(
        ...
        """
        Answers a question using the ainktechQA RAG asynchronously.
        """
        concat_list_files = format_file_list(
            list_files, self.retrieval_config.max_files
        )
