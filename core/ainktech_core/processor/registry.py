...
from ainktech_core.files.file import FileExtension
...
base_processors: ProcMapping = {
    FileExtension.txt: [
        ProcEntry(
            cls_mod="ainktech_core.processor.implementations.simple_txt_processor.SimpleTxtProcessor",
            err=None,
    ...
    FileExtension.pdf: [
        ProcEntry(
            cls_mod="ainktech_core.processor.implementations.tika_processor.TikaProcessor",
            err=None,
            priority=_LOWEST_PRIORITY,
        )
    ],
...
def defaults_to_proc_entries(
    ...
    for supported_extensions, processor_name in [
        ...
        for ext in supported_extensions:
            ...
            _append_proc_mapping(
                ...
                file_exts=[ext],
                cls_mod=f"ainktech_core.processor.implementations.default.{processor_name}",
                errtxt=f"can't import {processor_name}. Please install ainktech-core[{ext_str}] to access {processor_name}",
                priority=None,
            )
    ...
    _append_proc_mapping(
        ...
        cls_mod="ainktech_core.processor.implementations.megaparse_processor.MegaparseProcessor",
        errtxt=f"can't import MegaparseProcessor. Please install ainktech-core[{ext_str}] to access MegaparseProcessor",
        priority=None,
    )
...
def register_processor(
    ...
    if isinstance(proc_cls, str):
        ...
        ), f"{proc_cls} should be a subclass of ainktech_core.processor.ProcessorBase"
